...
import { CreateExperimentDto, RatingDto, RedemptionDto } from './dto.js';
...
export class PlatformService {
  ...
  async listExperiments(user: UserRow) {
    ...
    return rows.map(r => experiment(r, isResearcher));
  }
  async create(user: UserRow, dto: CreateExperimentDto) {
    ...
    const id = randomUUID();
    const row = (await this.db.query(`INSERT INTO experiments(id,owner_id,code,title,description,required_items,tags,tags_locales,location_type,location_detail,reward_points,duration_minutes,min_reputation_required,capacity)
  ...
  async enroll(user: UserRow, id: string) {
    student(user);
    return this.db.transaction(async client => {
      ...
      const existing = (await client.query('SELECT id FROM enrollments WHERE experiment_id=$1 AND user_id=$2', [id,user.id])).rows[0];
      ...
      return (await client.query('INSERT INTO enrollments(id,experiment_id,user_id,reward_points) VALUES($1,$2,$3,$4) RETURNING id', [randomUUID(),id,user.id,exp.reward_points])).rows[0];
    });
  }
  async profile(user: UserRow) {
    student(user);
    const current = (await this.db.query<UserRow>('SELECT * FROM users WHERE id=$1',[user.id])).rows[0];
    const rows = (await this.db.query('SELECT n.*,e.title FROM enrollments n JOIN experiments e ON e.id=n.experiment_id WHERE n.user_id=$1 ORDER BY n.created_at DESC LIMIT 500', [user.id])).rows;
  ...
  async subjects(user: UserRow) {
    ...
    const rows = (await this.db.query('SELECT n.*,e.title FROM enrollments n JOIN experiments e ON e.id=n.experiment_id WHERE e.owner_id=$1', [user.id])).rows;
  ...
  async enrollments(user: UserRow, id: string) {
    ...
    return (await this.db.query('SELECT n.id,n.user_id,n.status,n.reward_points,n.created_at,u.name FROM enrollments n JOIN users u ON n.user_id=u.id WHERE n.experiment_id=$1 ORDER BY n.created_at',[id])).rows;
  }
...
export class PlatformController {
  ...
  @Post('experiments/:id/enroll') enroll(@Req() req:AuthRequest,@Param('id',new ParseUUIDPipe()) id:string) { return this.service.enroll(req.user,id); }
